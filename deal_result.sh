...
val3=$((1+$j*9))
val4=$((9+$j*9))
sed -n ''$val3','$val4'p' $path.csv.end|awk 'BEGIN{FS=","};{if((double)strtonum($4)>0)printf("%d,%d,%.2lf,",$2,$5,(double)strtonum($2)/((double)strtonum($5)+0.01));else printf("%d,%d,%.2lf,",$2,$5,0)}'|sed 's/$/\n/g' >>$path.csv

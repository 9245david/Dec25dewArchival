...
echo 'netarr,netfin,arriv,num0,time,speed,netarr,netfin,arriv,num1,time,speed,netarr,netfin,arriv,num2,time,speed,netarr,netfin,arriv,num3,time,speed,netarr,netfin,arriv,num4,time,speed,netarr,netfin,arriv,num5,time,speed,netarr,netfin,arriv,num6,time,speed,netarr,netfin,arriv,num7,time,speed,netarr,netfin,arriv,num8,time,speed'>$path.csv
for i in $(seq 0 30)
do
val1=$((2+$i*9))
val2=$((10+$i*9))
sed -n ''$val1','$val2'p' $path|sed 's/[[:alpha:]]//g' |sed 's/_//g' |sed 's/^.//g'|sed 's/\s//g'|sort -k1 >$path.csv.temp
awk 'BEGIN{FS=","};{print $1","$4","$5","$7","$8","$9","$10}' $path.csv.temp >> $path.csv.end
done
for j in $(seq 0 30)
do
val3=$((1+$j*9))
val4=$((9+$j*9))
sed -n ''$val3','$val4'p' $path.csv.end|awk 'BEGIN{FS=","};{if((double)strtonum($3)>0)printf("%d,%d,%d,%d,%d,%.2lf,",$2,$3,$4,$5,$6,(double)strtonum($3)/((double)strtonum($6)+0.01));else printf("%d,%d,%d,%d,%d,%.2lf,",$2,$3,$4,$5,$6,0)}'|sed 's/$/\n/g' >>$path.csv

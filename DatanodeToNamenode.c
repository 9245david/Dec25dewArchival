...
int32_t g_unfinished_net = 0;
int32_t g_finished_net = 0;
int32_t g_allocated_net = 0;
...
//	assert(sock_DtoN > 0);
        ...
        g_allocated_net = RecvTaskOfNetBlockNum(task_length,recvTaskBuff);
		...
		if(g_allocated_net>0)g_unfinished_net = g_unfinished_net + g_allocated_net;
		...
		FeedbackDToN->unfinishedNet = g_unfinished_net;
		FeedbackDToN->finishedBlock = g_finished_block;
		FeedbackDToN->finishedNet = g_finished_net;
		FeedbackDToN->allocatedBlock = g_allocated_block;
		FeedbackDToN->allocatedNet = g_allocated_net;
		g_finished_task = 0;
                g_finished_block = 0;
                g_finished_net = 0;
		pthread_mutex_unlock(&g_finished_task_lock);
...
int32_t RecvTaskOfNetBlockNum(int32_t taskNum, char * recvBuff)
{
        pTaskBlock pChunkTask = NULL;
        int32_t blockNum = 0;
        int32_t i = 0;
       ...
       while(*(localBlock+localBlockNum) != -1)
                ...
                waitBlockNum = pChunkTask -> waitForBlock;
                destBlockNum = pChunkTask -> destIPNum;
                blockNum = blockNum + waitBlockNum + destBlockNum;
        }
        if(DEW_DEBUG >0)fprintf(stderr,"1-21 NetblockNum %d\n",blockNum);
	return blockNum;
}
